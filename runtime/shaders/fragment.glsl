...
uniform float m_fFrequencies0; // 0 - 60 Hz
uniform float m_fFrequencies1; // 60 - 150 Hz
uniform float m_fFrequencies2; // 150 Hz - 400 Hz
uniform float m_fFrequencies3; // 400 Hz - 1 KHz
uniform float m_fFrequencies4; // 1 KHz - 2.4 KHz
uniform float m_fFrequencies5; // 2.4 Khz - 15 KHz
uniform float m_fFrequencies6; // 15 KHz+
...
#define TIME        m_iTime
#define RESOLUTION  m_vec2Resolution
...
void main() {
  ...
  vec3 col = color(ww, uu, vv, ro, p) * (m_fFrequencies0*10);
  col = aces_approx(col);
  col = sqrt(col);
